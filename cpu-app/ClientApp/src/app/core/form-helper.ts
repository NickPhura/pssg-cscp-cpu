...
export class FormHelper {
  ...
  isFormValid(
    notificationQueueService: NotificationQueueService = null,
    currentTabHasInvalidClass: number = 0,
    ignoreTabErrors = false,
  ) {
    if (document.getElementsByClassName("ng-invalid").length > 0) {
      if (notificationQueueService)
        notificationQueueService.addNotification(
          "All fields must be in a valid format.",
          "warning",
        );
      return false;
    }
    if (
      !ignoreTabErrors &&
      document.getElementsByClassName("tab-invalid").length >
        currentTabHasInvalidClass
    ) {
      if (notificationQueueService)
        notificationQueueService.addNotification(
          "There is a problem on another tab preventing save.",
          "warning",
        );
      return false;
    }
  ...
  isDialogValid(notificationQueueService: NotificationQueueService = null) {
    if (
      document.querySelectorAll(".mat-dialog-container .ng-invalid").length > 0
    ) {
      if (notificationQueueService)
        notificationQueueService.addNotification(
          "All fields must be in a valid format.",
  ...
  getFormState() {
    ...
    if (dirtyControls.length > 0) {
      for (let i = 0; i < dirtyControls.length; ++i) {
        if (
          dirtyControls[i].classList.contains("ng-untouched") ||
          dirtyControls[i].classList.contains("reporting-month")
        )
          continue;
        ++count;
      }
  ...
  moneyMaskToNumber(
    ...
    varName,
    max: number = 0,
    alwaysEnforceMax: boolean = false,
  ) {
    if (!max) max = 0;
