...
import {
  iDynamicsCrmContact,
  iDynamicsScheduleFResponse,
  iDynamicsServiceArea,
} from "./dynamics-blob";
...
export class TransmogrifierProgramApplication {
  ...
  private buildAdministrativeInformation(
    b: iDynamicsScheduleFResponse,
  ): iAdministrativeInformation {
    return {
      ccseaMemberType: decodeCcseaMemberType(
        parseInt(b.Contract.vsd_cpu_memberofcssea),
      ),
      ...
      staffSubcontracted: b.Contract.vsd_cpu_subcontractedprogramstaff
        ? b.Contract.vsd_cpu_subcontractedprogramstaff === boolOptionSet.isTrue
          ? true
          : false
        : null,
      staffUnionized: b.Contract.vsd_cpu_unionizedstaff
  ...
  private buildContactInformation(
    ...
    const c: iContactInformation = {
      ...
      hasMailingAddress: !!(
        b.Organization.address2_city ||
        b.Organization.address2_line1 ||
        b.Organization.address2_line2 ||
        b.Organization.address2_stateorprovince ||
        b.Organization.address2_postalcode
      ),
    };
    ...
    if (
      b.BoardContact &&
      b.Contract._vsd_contactlookup1_value !==
        b.Contract._vsd_contactlookup2_value
    ) {
      c.boardContact = {
  ...
  private buildProgramApplications(
    ...
    for (let p of g.ProgramCollection) {
      let temp: iProgramApplication = {
        ...
        serviceAreas: g.ServiceAreaCollection.filter(
          (sa: iDynamicsServiceArea): boolean =>
        ...
        programContact:
          g.StaffCollection.filter(
            (c: iDynamicsCrmContact): boolean =>
              p._vsd_contactlookup_value === c.contactid,
          ).map((s) => this.makePerson(g, s.contactid))[0] || null,

        policeContact:
          g.StaffCollection.filter(
            (c: iDynamicsCrmContact): boolean =>
              p._vsd_contactlookup2_value === c.contactid,
          ).map((s) => this.makePerson(g, s.contactid))[0] || new Person(),

        sharedCostContact:
          g.StaffCollection.filter(
            (c: iDynamicsCrmContact): boolean =>
              p._vsd_contactlookup3_value === c.contactid,
          ).map((s) => this.makePerson(g, s.contactid))[0] || new Person(),
        ...
        additionalStaff:
          g.ProgramContactCollection.filter(
            (c: iDynamicsCrmContact) => c.vsd_programid === p.vsd_programid,
          ).map((s) => this.makePerson(g, s.contactid)) || null, // iPerson[];
        subContractedStaff:
          g.ProgramSubContractorCollection.filter(
            (c: iDynamicsCrmContact) => c.vsd_programid === p.vsd_programid,
          ).map((s) => this.makePerson(g, s.contactid)) || null, // iPerson[];
        operationHours: [],
        standbyHours: [],
      ...
      let programType = g.ProgramTypeCollection.find(
        (pt) => pt.vsd_programtypeid === p._vsd_programtype_value,
      );
      temp.isPoliceBased = programType
        ? programType.vsd_programcategory === 100000000
        : false;
      temp.assignmentArea =
        g.RegionDistrictCollection.filter(
          (x) => p._vsd_cpu_regiondistrict_value === x.vsd_regiondistrictid,
  ...
  private makePerson(g: iDynamicsScheduleFResponse, personId: string): iPerson {
    // return whole person
    return g.StaffCollection.filter(
      (p: iDynamicsCrmContact) => p.contactid === personId,
